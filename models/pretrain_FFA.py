...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = UnetGen(input_nc, output_nc)
    elif netG == 'CBAM':
        net = ResnetGeneratorCBAM(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    else:
...
def visualize_feature():
    ...
    for i in range(img.size(0)):
        image = img[i]
        # print(image.size())
        image = transform1(numpy.uint8(image.numpy().transpose(1, 2, 0)))
        image.show()
...
class Pix2PixFFAModel(nn.Module):
    def __init__(self, device, ckpt_G, ckpt_D):
        super(Pix2PixFFAModel, self).__init__()

        self.ckpt_D = ckpt_D
        self.ckpt_G = ckpt_G
        self.device = device
        self.netG_A = UnetGen(3, 1)
        self.netG_A.load_state_dict(torch.load(self.ckpt_G))
        self.feature_extractor = nn.Sequential(self.netG_A.down6, self.netG_A.down7, self.netG_A.up1, self.netG_A.up2, self.netG_A.up3, self.netG_A.up4, self.netG_A.up5, self.netG_A.up6, self.netG_A.up7)
        self.gan_head = nn.Sequential()
       # if self.isTrain:  # define a discriminator; conditional GANs need to take both input and output images; Therefore, #channels for D is input_nc + output_nc
        self.netD_A = NLayerDiscriminator(4, 64)
        self.netD_A.load_state_dict(torch.load(self.ckpt_D))
    ...
    def forward(self, x):
        ...
        self.real_A = x
        self.fake_B, feature = self.netG_A(x)  # G(A)
        fake_AB = torch.cat((self.real_A, self.fake_B), 1)
        pred_fake = self.netD_A(fake_AB)
        return feature, pred_fake#, self.fake_B
    ...
    def backward_D(self):
     ...
     #   pred_fake = pred_fake.repeat(1,3, 1, 1)
        self.loss_D_fake = self.criterionGAN(pred_fake, False)
        self.loss_D_fake.backward()


    def backward_G(self, loss):
        ...
        self.loss_G = 0.3*self.loss_G_GAN + loss
        self.loss_G.backward(retain_graph=True)
        # print(self.netG_A.down6.do[0].weight.grad)
    ...
    def optimize_parameters(self, x, loss, optimizer_G):
        ...
        self.backward_G(loss)                   # calculate graidents for G
